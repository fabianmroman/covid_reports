...
""" Carga "artesanal" para marzo, desde el 29/03 tiene el formato actual y el dataset se crea
	con este script. Para dias anteriores se cargaron a mano los datos a partir de los PDFs bajados.
	Antes se utilizaron dos formas de publicas los datos:
    del 16 o 17 hasta el 28/03 tiene un formato intermedio entre el actual y el inicial
    del 16 para atras estan los datos en un unico parrafo.
    El 24/03 comienzan a publicarse los reportes matutino y vespertino. El informe vespertino tiene
	informacion completa del dia y el informe matutino es informacion del dia anterior mas una 
	actualizacion de la cantidad de decesos.
	Las cifras se ajustaron al 29/03/20. A partir de ese momento comenzo a disponerse de datos
	completos. Durante los primeros dias de marzo no se especificaban la cantidad de casos por
	cada jurisdiccion.
"""
    ...
    for line in content:
        if "pdf" in line:
            ...
            if "VESPERTINO" in filename.upper():
                ...
                if diasanteriores != []:
                    for dia in diasanteriores:
                        urlretrieve(dia[0], dia[1])
                        listadodelmesdecr.append(dia[1])
                urlretrieve(URL, filename)
                listadodelmesdecr.append(filename)
                diasanteriores = []
            elif "MATUTINO" in filename.upper():
                diasanteriores.append([URL, filename])
            else:
                if diasanteriores != []:
                    dia = diasanteriores[0]
                    urlretrieve(dia[0], dia[1])
                    listadodelmesdecr.append(dia[1])
                    diasanteriores = []
                else:
...
for pdf_file in listadoarchivos:
    ...
    if "matutino" in pdf_file:
        if pdf_file == "covid19_informe-diario-matutino-24-03_0.pdf":
            fecha = ["24","03",anio]
